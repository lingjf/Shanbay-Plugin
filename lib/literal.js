...
function hasEnglish(w) {
	for(var i = 0; i < w.length; i++) {
		if(isEnglish(w.charAt(i))) {
			return true;
		}
	}
	return false;
...
function getWildcards(word, maximum) {
	var result = [];
	var c = 0;

	var re = new RegExp('^' + word.replace(/\*/g, '.*').replace(/\?/g, '.') + '$', "i");
...
function getPhontics1(word, maximum) {
	var m = metaphone(word);
	var h = [];
	for (var i = 0; i < wordlist.length; i++) {
		if (metaphone(wordlist[i]) == m) {
			h.push(wordlist[i]);
		}
	}
	return h.map(function(d){return {word:d};});
}

function getPhontics(word, maximum) {
	var m = DoubleMetaphone(word);
	var h = [];
	for (var i = 0; i < wordlist.length; i++) {
		var n = DoubleMetaphone(wordlist[i])
		if (m[0] == n[0] || m[1] == n[1]) {
			h.push(wordlist[i]);
		}
	}
...
*/　
	...
	if (hasEnglish(ss) && hasChinese(ss)) {
		return true;
	}
	return false;
}
...
function splitParagraph(ss) {
	var result = [];
	var duplicated = {};
	// var t = ss.match(/[a-z\u4e00-\u9fa5]+\s*/gim);
	// var t = ss.match(/(?:[a-z]+(?:\'s)?[a-z]*)|(?:[\u4e00-\u9fa5]+)/gim);
	var t = ss.match(/(?:[a-z]+)|(?:[\u4e00-\u9fa5]+)/gim);
...
function classifyText(s) {
	if (isPhrase(s)) {
		...
		return "wildcard";
	} else if (areChinese(s)) {
		return "chinese";
	} else if (isParagraph(s)) {
		return "paragraph";
	} else {
		return "english";
	}
